...
async def report_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    thread_id = update.message.message_thread_id if update.message.is_topic_message else None
    if update.effective_user.id != OWNER_ID: return
    await update.message.reply_text("📢 گزارش کامل به پیوی مالک ارسال گردید.", message_thread_id=thread_id)
...
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "mode_random":
        ...
        save_db()
        asyncio.create_task(start_auto_sending(chat_id, thread_id, context))
        await update.message.reply_text("🚀 اتک تصادفی استارت خورد!")
    elif data == "mode_lock":
        bot_data["attack_mode"] = "lock"
        bot_data["is_running"] = True
        save_db()
        asyncio.create_task(start_auto_sending(chat_id, thread_id, context))
        await update.message.reply_text("🔴 اتک قفلی استارت خورد!")
    elif data == "stop_attack":
        bot_data["is_running"] = False
        save_db()
        await update.message.reply_text("🛑 اتک متوقف شد.")
