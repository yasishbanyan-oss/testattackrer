...
# لینک WebApp (وقتی فایل index.html رو تو گیت‌هاب گذاشتی، آدرس رندرش رو اینجا بگذار)
WEBAPP_URL = os.environ.get("WEBAPP_URL", "https://your-render-app.onrender.com/index.html")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "menu_main":
        ...
        for aid, ainfo in bot_data["admins"].items():
            ...
            if uname != "OWNER" and uname != "نامشخص":
                uname = f"@{uname}"
        ...
        help_text = (
            ...
            "/go - شروع اتک\n"
            "/stop - توقف اتک\n"
            "/report - گزارش زنده ربات به پیوی مالک\n"
            "/info - (عمومی) مشخصات کامل کاربر بر روی ریپلی\n"
        )
        await query.edit_message_text(help_text, parse_mode="Markdown", reply_markup=get_main_menu(owner_user_id))
        return ConversationHandler.END

# --- دریافت دیتا از WebApp (دکمه‌های رنگی) ---
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "mode_random":
        ...
        save_db()
        asyncio.create_task(start_auto_sending(chat_id, thread_id, context))
        await update.message.reply_text("🚀 اتک با حالت **تصادفی (Random)** استارت خورد!")
        ...
        save_db()
        asyncio.create_task(start_auto_sending(chat_id, thread_id, context))
        await update.message.reply_text("🔴 اتک **قفلی (Lock)** استارت خورد!")
...
# --- گرفتن اطلاعات کاربر (/info) بر روی ریپلی ---
async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if photos.total_count > 0:
            await update.message.reply_photo(photo=photos.photos[0][-1].file_id, caption=info_text, parse_mode="Markdown", message_thread_id=thread_id)
            return
    except Exception:
        pass
...
# --- گزارش زنده (/report) همراه با اسم گروه‌ها ---
async def report_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    thread_id = update.message.message_thread_id if update.message.is_topic_message else None
    if update.effective_user.id != OWNER_ID: return
    
    await update.message.reply_text("📢 گزارش کامل وضعیت ربات به پیوی مالک ارسال گردید.", message_thread_id=thread_id)
    
    groups_list_text = "👥 **گروه‌های فعال ربات:**\n"
    joined_groups = bot_data.get("joined_groups", {})
    
    if joined_groups:
        for gid, gtitle in joined_groups.items():
            groups_list_text += f"• {gtitle} (`{gid}`)\n"
    else:
        groups_list_text += "هیچ گروهی ثبت نشده است."
...
# --- دریافت آیدی ادمین جدید و استخراج یوزرنیم ---
async def receive_admin_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.isdigit():
        ...
        await update.message.reply_text("❌ آیدی عددی معتبر وارد کنید:", message_thread_id=thread_id)
        return WAITING_FOR_ADMIN_ID

# --- هندر لاگ و ذخیره گروه‌ها ---
...
async def start_auto_sending(chat_id: int, thread_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    while bot_data["is_running"]:
        ...
        for uid, uinfo in bot_data["saved_users"].items():
            tag_word = uinfo.get("custom_tag") or default_tag
            tags_list.append(f"[{tag_word}](tg://user?id={uid})")
...
# --- سرو فایل index.html توسط وب‌سرور داخلی رندر ---
